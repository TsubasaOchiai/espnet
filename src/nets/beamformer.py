...
def _flatten(outer_list):
    flens = [len(inner) for inner_list in outer_list]
...
def _batch_inv_complex(a, coef=1e-10):
    ...
    # n x n
    E = coef * np.eye(a_real[-1], dtype=np.float32)
    E = chainer.Variable(xp.array(E, dtype=np.float32))
...
def _batch_trace_complex(a):
    ...
    # n x n
    E = np.eye(a_real[-1], dtype=np.float32)
    E = chainer.Variable(xp.array(E, dtype=np.float32))
...
class NB_MVDR(chainer.Chain):
    def __init__(self, bidim, args):
        ...
        with self.init_scope():
            self.mask = MaskEstimator(args.btype, bidim, args.blayers, args.bunits, args.bprojs, args.dropout)
            self.ref = AttRef(bidim, args.bunits, args.bprojs, args.badim)
    ...
    def __call__(self, data):
        ...
        psds = {}
        psds['speech'] = _get_psd_SN(data, masks['speech'])
        psds['noise'] = _get_psd_SN(data, masks['noise'])

        # utt x channel
        u = self.ref(psds['speech'], hs)

        # utt list of freq x channel
        ws = _get_filter(psds, u)

        # utt list of frame x freq
        es = _filtering(data, ws)
    ...
    def _get_filter(self, psds, u):
        ...
        us['imag'] = chainer.Variable(self.xp.array(
                        np.zeros(u_real.shape, dtype=np.float32), dtype=np.float32))
...
class MaskEstimator(chainer.Chain):
    ...
    def __call__(self, xs):
        ...
        xs,flens = _flatten(xs)  # utt channel list of frame x 2 freq
        ilens = np.array([xx.shape[0] for xx in xs] dtype=np.int32)
        xs, ilens = self.blstm(xs, ilens)  # utt channel list of frame x bprojs
        xs = F.vstack(xs)  # (utt channel * frame) x bprojs
        # speech
        ...
        # noise 
        ms_N = F.sigmoid(self.lo_N(xs))
        ms_N = F.split_axis(ms_N, np.cumsum(ilens[:-1]), axis=0)
        ms_N = _deflatten(ms_N, flens)
        ...
        xs = F.split_axis(xs, np.cumsum(ilens[:-1]), axis=0)  # utt channel list of frame x bprojs
        xs = _deflatten(xs, flens)  # utt list of channel list of frame x bprojs
...
class AttRef(chainer.Chain):
    def __init__(self, bidim, bunits, bprojs, att_dim, dropout):
        ...
        with self.init_scope():
            ...
            self.mlp_state = L.Linear(brojs, att_dim, nobias=True)
            self.gvec = L.Linear(att_dim, 1)

    def __call__(self, psd_in, state_in):
        # psd_feat
        # utt x freq x channel x channel
        psd_real = psd_in['real']
        psd_imag = psd_in['iamg']
        cdim = psd_real.shape[-1]
        # utt x freq x channel
        psd_real = F.sum(_diag_zeros(psd_real)) / (cdim - 1)
        psd_imag = F.sum(_diag_zeros(psd_imag)) / (cdim - 1)
        ...
        # utt x channel x att_dim -> utt x channel
