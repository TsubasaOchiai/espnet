...
class SeqEvaluaterKaldi(extensions.Evaluator):
    def __init__(self, iterator, target, readers, device):
    ...
    def evaluate(self):
        ...
        for batch in it:
            ...
            with reporter_module.report_scope(observation):
                ...
                x = converter_kaldi(batch[0], self.readers)
...
class SeqUpdaterKaldi(training.StandardUpdater):
    def __init__(self, train_iter, optimizer, readers, device):
...
def converter_kaldi(batch, readers):
    data = batch[0]
    mode = data[1]['mode']
...
def main():
    ...
    # Set up a trainer
    updater = SeqUpdaterKaldi(train_iter, optimizer, train_readers, gpu_id)
    ...
    trainer.extend(SeqEvaluaterKaldi(
        valid_iter, model, valid_readers, device=gpu_id))
